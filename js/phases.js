...
class PhaseBase {
  ...
  /**
   * Fragmento Ancestral — colecionável opcional (um por fase, 5 no total).
   * Nunca bloqueia a conclusão da fase; só conta para o Final Secreto
   * (Evolução). Usa o mesmo Item/spawnScattered de todo o resto do jogo.
   */
  updateCollectibles(){
    const p = this.game.player;
    this.items = this.items.filter(it => {
      ...
      if(Math.hypot(p.x - it.x, p.y - it.y) < p.r + it.r){
        ...
        this.showMessage('Fragmento Ancestral encontrado!');
        return false;
      }
      return true;
    });
  ...
  renderItems(ctx){
    ...
    this.items.forEach(it => {
      ...
      ctx.shadowBlur = it.state === 'carried' ? 10 : (it.type === 'core' ? 14 : 4);
      ...
      if(it.type === 'core'){
        ...
        ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.arc(x, y, it.r + 5, 0, Math.PI * 2);
        ctx.stroke();
        ctx.globalAlpha = 1;
      }
      if(it.state === 'carried'){
